...
const AreaEntry landAreaMap[89] = {
...
const AreaEntry waterAreaMap[49] = {
...
const AreaEntry rockAreaMap[12] = {
...
int main() {
    ...
    const char *path = get_encounter_file_path(LG, ROCKSMASH);
    Slot *slots = load_slots(rockAreaMap[10], path);

    for (int i = 0; i < ROCK_SLOTS; i++) {
        printf("Slot %d -> Index: %d, Mon: %d, MinL: %d, MaxL: %d\n",
               i, slots[i].index, slots[i].mon, slots[i].minL, slots[i].maxL);
    }
    free(slots);
